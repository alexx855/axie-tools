...
import { parseEther, Wallet, Contract } from "ethers";
...
liveTest(
  ...
  async () => {
    ...
    if (!PRIVATE_KEY || !SKYMAVIS_API_KEY || !MATERIAL_ID || !MAX_TOTAL_COST) {
      throw new Error(
        "Missing required env vars: PRIVATE_KEY, SKYMAVIS_API_KEY, MATERIAL_ID, MAX_TOTAL_COST",
      );
    }
